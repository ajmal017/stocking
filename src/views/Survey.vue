<template>
  ...
  <form @submit.prevent="submitSurvey">
    <div
      v-if="tab === 0"
      class="my-5 text-center animate__animated animate__backInLeft"
      v-bind:class="{ animate__backOutLeft: tabChange }"
    >
      <img
        src="../assets/img/cake.png"
        class="mx-auto d-block img-fluid mb-4"
      />
      <p class="form-label text-white fs-4">How old are you?</p>
      <div class="row mt-5">
        <div
          v-for="(option, index) in ageOption"
          :key="index"
          class="col-12 mb-3"
        >
          <input
            ...
            @click="changeTab(1)"
          />
          <label class="btn btn-lg btn-outline-light" :for="option.name">{{
            option.name
          }}</label>
        </div>
      </div>
    ...
    >
      <img
        ...
        class="mx-auto d-block img-fluid mb-4"
      />
      <p class="form-label text-white fs-4">
        How long is your trading experience?
      </p>
      <div class="row mt-5">
        <div
          v-for="(option, index) in tradingExpOption"
          :key="index"
          class="col-12 mb-3"
        >
          <input
    ...
    <div
      v-if="tab === 3"
      class="text-only my-5 text-center animate__animated animate__backInLeft"
      v-bind:class="{ animate__backOutLeft: tabChange }"
    >
      <p class="form-label text-white fs-4">
        Describe your expectations in the field of investment.
      </p>
      <textarea
    ...
    >
      <p class="form-label text-white fs-4">
        If you are irritated by somebody, what would you do?
      </p>
      <textarea
        v-model="irritated"
        class="form-control mt-5 mb-4"
    ...
    >
      <p class="form-label text-white fs-4">
        How do you deal with stress in your everyday life?
      </p>
      <textarea
        v-model="stress"
        class="form-control mt-5 mb-4"
    ...
    <button
      v-if="tab !== 0"
      class="bottom text-white animate__animated animate__backInLeft"
      v-on:click.prevent="changeTab(tab - 1)"
    >
      Back
    </button>
  </form>
...
export default {
  data() {
    return {
      url:
        "https://api.us-south.personality-insights.watson.cloud.ibm.com/instances/36eeaf67-af1a-4675-ae1d-0e998b3469ce/v3/profile?version=2020-11-21",
      tab: 0,
      tabChange: false,
      errors: [],
      age: "",
  ...
  methods: {
    ...
    async submitSurvey() {
      ...
      const headers = {
        Authorization:
          "Basic YXBpa2V5OmV3bGhwWFlpUk9FRVk5UlJMbHAzN3U3aUp4d3Z2VjRkUXJPU1BtRUNoS1cw",
