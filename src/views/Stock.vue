<template>
  <div
    ...
    v-bind:class="{ animate__fadeOutDownBig: back }"
  >
    <div class="row">
      ...
      <tbody>
        <tr v-for="(stock, index) in stocks" :key="index">
          ...
          <td
            ...
            v-bind:class="{
              'text-success': stock.shariah,
              'text-danger': !stock.shariah,
            }"
          >
          ...
          <td
            class="fw-bold text-center"
            v-bind:class="{
              'text-success': stock.increase,
              'text-danger': !stock.increase,
            }"
          >
            {{ stock.price }}
          </td>
...
export default {
  ...
  methods: {
    ...
    calculatePCF() {
      let newStocks = stocks.map((stock) => {
        stock.pcf =
          Math.round((stock.price / (stock.cashflow / stock.share)) * 100) /
          100;
        return stock;
      });
      this.stocks = newStocks;
    ...
    calculateDebtRatio() {
      let newStocks = stocks.map((stock) => {
        stock.debtRatio =
          Math.round((stock.bankBorrowings / stock.totalAsset) * 100) / 100;
        return stock;
      });
      this.stocks = newStocks;
    },
